...
<div class="tool-options">
  ...
  {#if globalState.image.data}
    <Button text="Clear overlay" onclick={() => (globalState.image.data = null)} />
  {/if}
  Scale
  <IncrementInput min={0.01} max={10} bind:value={globalState.image.scale} increments={0.1} />
  Opacity
  <Slider formatFunction={sliderFormat} min={0} max={1} bind:value={globalState.image.opacity} />
</div>
